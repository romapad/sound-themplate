...
<div id="reviews">
	...
	<h2 class="contributions-title"><?php _e( 'Share your thoughts!', WC_Product_Reviews_Pro::TEXT_DOMAIN ); ?></h2>

	<?php // Product ratings ?>
	<?php if ( 'yes' == get_option( 'woocommerce_enable_review_rating' ) && $product->get_rating_count() ) : ?>
	<div class="product-rating">
		<div class="product-rating-summary">
			<h3><?php printf( __( '%s out of %d stars', WC_Product_Reviews_Pro::TEXT_DOMAIN ), floatval( $product->get_average_rating() ), 5 ); ?></h3>

			<?php $reviews_count = wc_product_reviews_pro_get_comments_number( $product->id, 'review' ); ?>
			<p><?php printf( _nx( '%d review', '%d reviews', $reviews_count, 'noun', WC_Product_Reviews_Pro::TEXT_DOMAIN ), $reviews_count ); ?></p>
		</div>
	...
	<h3 class="contributions-form-title"><?php _e( 'Let us know what you think...', WC_Product_Reviews_Pro::TEXT_DOMAIN ); ?></h3>
	...
	<?php if ( ! is_user_logged_in() && get_option('comment_registration') ) : ?>
		<noscript>
			...
			<p class="must-log-in"><?php printf( __( 'You must be <a href="%s">logged in</a> to join the discussion.', WC_Product_Reviews_Pro::TEXT_DOMAIN ), esc_url( add_query_arg( 'redirect_to', urlencode( get_permalink( get_the_ID() ) ), SV_WC_Plugin_Compatibility::wc_get_page_permalink( 'myaccount' ) . '#tab-reviews' ) ) ); ?></p>
		</noscript>
	<?php endif; ?>
	...
	<div id="comments">
		...
		<form method="get" action="#comments" class="contributions-filter">
			...
			// Filter options
			$options = array(
				'' => __( 'Show everything', WC_Product_Reviews_Pro::TEXT_DOMAIN ),
			);
			...
			<a href="<?php the_permalink(); ?>" class="js-clear-filters" style="display:none;" title="<?php _e( 'Click to clear filters', WC_Product_Reviews_Pro::TEXT_DOMAIN ); ?>"><?php _e( '(clear)', WC_Product_Reviews_Pro::TEXT_DOMAIN ); ?></a>

			<?php woocommerce_form_field( 'comments_filter', $args, $comments_filter ); ?>

			<noscript><button type="submit" class="button"><?php _e( 'Go', WC_Product_Reviews_Pro::TEXT_DOMAIN ); ?></button></noscript>
		</form>
	...
	<div id="wc-product-reviews-pro-modal">
		...
		<div class="switcher">
			<p class="login"><?php printf( __( 'Already have an account? %sLog In%s', WC_Product_Reviews_Pro::TEXT_DOMAIN ), '<a href="#">', '</a>' ); ?></p>
			<p class="register"><?php printf( __( 'Don\'t have an account? %sSign Up%s', WC_Product_Reviews_Pro::TEXT_DOMAIN ), '<a href="#">', '</a>' ); ?></p>
		</div>
	</div>

...
<?php if ( 'review' != $type || get_option( 'woocommerce_review_rating_verification_required' ) === 'no' || wc_customer_bought_product( '', get_current_user_id(), $product->id ) ) : ?>
	...
	<?php if ( 'contribution_comment' == $type && ! is_user_logged_in() && get_option('comment_registration') ) : ?>
		<noscript>
			...
			<p class="must-log-in"><?php printf( __( 'You must be <a href="%s">logged in</a> to join the discussion.', WC_Product_Reviews_Pro::TEXT_DOMAIN ), esc_url( add_query_arg( 'redirect_to', urlencode( get_permalink( get_the_ID() ) ), SV_WC_Plugin_Compatibility::wc_get_page_permalink( 'myaccount' ) . '#comment-' . $comment->comment_ID ) ) ); ?></p>
		</noscript>
	<?php endif; ?>
